...
# 直接讀取，副檔名不是問題
csi_data = csiread.Intel("data/csi_3min_1min2min.dat")
csi_data.read()
...
def lowpass_filter(data, cutoff=10, fs=5, order=5):
    """
...
# 對所有子載波進行低通濾波
amp_filtered = lowpass_filter(amp, cutoff=0.5, fs=5)
...
np.save("csi_3min_1min2min/csi_3min_1min2min.npy", csi)